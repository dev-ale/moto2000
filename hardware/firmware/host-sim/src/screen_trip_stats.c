                                ...
                                uint8_t                      flags)
{
    ...
    host_sim_draw_text(canvas, dist_buf, dist_x, dist_y, dist_scale,
                       fg_r, fg_g, fg_b);
    ...
    host_sim_draw_text(canvas, dur_buf, dur_x, dur_y, dur_scale,
                       accent_r, accent_g, accent_b);

    /* ---------------- avg / max speed cells ---------------- */
    char avg_buf[16];
    char max_buf[16];
    (void)host_sim_format_speed_cell("AVG", data->average_speed_kmh_x10,
                                     avg_buf, sizeof(avg_buf));
    (void)host_sim_format_speed_cell("MAX", data->max_speed_kmh_x10,
    ...
    char asc_buf[12];
    char dsc_buf[12];
    (void)host_sim_format_elevation_delta(data->ascent_meters, 0,
                                          asc_buf, sizeof(asc_buf));
    (void)host_sim_format_elevation_delta(data->descent_meters, 1,

...
static void draw_line(host_sim_canvas_t *canvas,
...
static void fill_triangle(host_sim_canvas_t *canvas,
                          int x0, int y0,
                          int x1, int y1,
                          int x2, int y2,
                          uint8_t r, uint8_t g, uint8_t b)
{
    ...
    const int64_t denom =
        (int64_t)(y1 - y2) * (x0 - x2) + (int64_t)(x2 - x1) * (y0 - y2);
    if (denom == 0) {
        return;
    }
    for (int py = min_y; py <= max_y; ++py) {
        for (int px = min_x; px <= max_x; ++px) {
            const int64_t a_num =
                (int64_t)(y1 - y2) * (px - x2) + (int64_t)(x2 - x1) * (py - y2);
            const int64_t b_num =
                (int64_t)(y2 - y0) * (px - x2) + (int64_t)(x0 - x2) * (py - y2);
...
static void draw_ring(host_sim_canvas_t *canvas,
                      int cx, int cy,
...
static void draw_ticks(host_sim_canvas_t *canvas,
                       ...
                       uint8_t  b)
{
    for (int deg = 0; deg < 360; deg += 10) {
        ...
        const compass_point_t outer =
            host_sim_compass_point_on_dial(heading_deg_x10,
                                           (uint16_t)(deg * 10),
                                           cx, cy, outer_radius);
...
static void draw_cardinal_labels(host_sim_canvas_t *canvas,
                                 uint16_t heading_deg_x10,
                                 int      cx,
                                 int      cy,
                                 int      label_radius,
                                 ...
                                 uint8_t  b)
{
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        const compass_point_t p =
            host_sim_compass_point_on_dial(heading_deg_x10, tick_x10,
                                           cx, cy, label_radius);
        const int w = host_sim_measure_text(labels[i], scale);
        host_sim_draw_text(canvas,
                           labels[i],
                           p.x - w / 2,
                           p.y - glyph_h / 2,
                           scale,
...
void host_sim_render_compass(host_sim_canvas_t        *canvas,
                             ...
                             uint8_t                   header_flags)
{
    ...
    const uint16_t displayed_x10 = host_sim_compass_displayed_heading_x10(
        compass->magnetic_heading_deg_x10,
        compass->true_heading_deg_x10,
        compass->compass_flags);

    draw_ticks(canvas, displayed_x10, cx, cy, outer_radius - 2,
               ring_r, ring_g, ring_b);
    draw_cardinal_labels(canvas, displayed_x10, cx, cy, label_radius,
                         ring_r, ring_g, ring_b);
    ...
    const bool         true_known =
        compass->true_heading_deg_x10 != BLE_COMPASS_TRUE_HEADING_UNKNOWN;
    ...
    host_sim_draw_text(canvas, text_buf,
                       cx - readout_w / 2,
                       readout_y,
                       readout_scale,
                       txt_r, txt_g, txt_b);
    host_sim_draw_text(canvas, acc_buf,
                       cx - acc_w / 2,
                       acc_y,
                       acc_scale,
                       txt_r, txt_g, txt_b);

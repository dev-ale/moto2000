...
int main(int argc, char **argv)
{
    ...
    s_sdl.window = SDL_CreateWindow("ScramScreen LVGL Simulator", SDL_WINDOWPOS_CENTERED,
                                    SDL_WINDOWPOS_CENTERED, DISPLAY_W, DISPLAY_H,
    ...
    s_sdl.renderer =
        SDL_CreateRenderer(s_sdl.window, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
    if (!s_sdl.renderer) {
    ...
    s_sdl.texture = SDL_CreateTexture(s_sdl.renderer, SDL_PIXELFORMAT_ARGB8888,
    ...
    while (running) {
        ...
        while (SDL_PollEvent(&ev)) {
            ...
            if (ev.type == SDL_KEYDOWN) {
                ...
                /* Arrow keys: cycle through screens using the last payload's
                 * data but with a different screen ID. */
                if (ev.key.keysym.sym == SDLK_LEFT) {
                    screen_manager_prev_screen();
                }
            ...
            if (select(1, &fds, NULL, NULL, &tv) > 0) {
                ...
                if (fgets(line, (int)sizeof(line), stdin) != NULL) {
                    ...
                    if (hex_to_bytes(line, payload, sizeof(payload), &payload_len) == 0) {
                        screen_manager_update_live(payload, payload_len);
                    } else {

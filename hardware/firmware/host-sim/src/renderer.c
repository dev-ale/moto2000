...
host_sim_canvas_t *host_sim_canvas_create(void)
{
    ...
    const size_t bytes = (size_t)c->width * (size_t)c->height * 3U;
...
int host_sim_render_payload(host_sim_canvas_t *canvas, const uint8_t *payload, size_t length)
{
    ...
    if (hdr != BLE_OK) {
        fprintf(stderr, "host-sim: failed to decode header: %s\n", ble_result_name(hdr));
    ...
    case BLE_SCREEN_COMPASS: {
        ...
        uint8_t flags = 0;
        const ble_result_t res = ble_decode_compass(payload, length, &flags, &compass);
        if (res != BLE_OK) {
            fprintf(stderr, "host-sim: failed to decode compass body: %s\n", ble_result_name(res));
            host_sim_canvas_fill(canvas, 200, 0, 0);
            host_sim_canvas_apply_round_mask(canvas);
            return 3;
        }
    ...
    case BLE_SCREEN_CLOCK: {
        ...
        uint8_t flags = 0;
        const ble_result_t res = ble_decode_clock(payload, length, &flags, &clock);
        if (res != BLE_OK) {
            fprintf(stderr, "host-sim: failed to decode clock body: %s\n", ble_result_name(res));
            host_sim_canvas_fill(canvas, 200, 0, 0);
            host_sim_canvas_apply_round_mask(canvas);
            return 3;
        }
    ...
    case BLE_SCREEN_SPEED_HEADING: {
        ...
        uint8_t flags = 0;
        const ble_result_t res = ble_decode_speed_heading(payload, length, &flags, &speed);
        if (res != BLE_OK) {
            fprintf(stderr, "host-sim: failed to decode speed+heading body: %s\n",
                    ble_result_name(res));
            host_sim_canvas_fill(canvas, 200, 0, 0);
            host_sim_canvas_apply_round_mask(canvas);
            return 3;
    ...
    case BLE_SCREEN_TRIP_STATS: {
        ...
        uint8_t flags = 0;
        const ble_result_t res = ble_decode_trip_stats(payload, length, &flags, &stats);
        if (res != BLE_OK) {
            fprintf(stderr, "host-sim: failed to decode trip stats body: %s\n",
                    ble_result_name(res));
            host_sim_canvas_fill(canvas, 200, 0, 0);
            host_sim_canvas_apply_round_mask(canvas);
            return 3;
    ...
    case BLE_SCREEN_WEATHER: {
        ...
        uint8_t flags = 0;
        const ble_result_t res = ble_decode_weather(payload, length, &flags, &weather);
        if (res != BLE_OK) {
            fprintf(stderr, "host-sim: failed to decode weather body: %s\n", ble_result_name(res));
            host_sim_canvas_fill(canvas, 200, 0, 0);
            host_sim_canvas_apply_round_mask(canvas);
            return 3;
        }
    ...
    case BLE_SCREEN_LEAN_ANGLE: {
        ...
        uint8_t flags = 0;
        const ble_result_t res = ble_decode_lean_angle(payload, length, &flags, &lean);
        if (res != BLE_OK) {
            fprintf(stderr, "host-sim: failed to decode lean angle body: %s\n",
                    ble_result_name(res));
            host_sim_canvas_fill(canvas, 200, 0, 0);
            host_sim_canvas_apply_round_mask(canvas);
            return 3;
    ...
    case BLE_SCREEN_MUSIC: {
        ...
        uint8_t flags = 0;
        const ble_result_t res = ble_decode_music(payload, length, &flags, &music);
        if (res != BLE_OK) {
            fprintf(stderr, "host-sim: failed to decode music body: %s\n", ble_result_name(res));
            host_sim_canvas_fill(canvas, 200, 0, 0);
            host_sim_canvas_apply_round_mask(canvas);
            return 3;
        }
    ...
    case BLE_SCREEN_APPOINTMENT: {
        ...
        uint8_t flags = 0;
        const ble_result_t res = ble_decode_appointment(payload, length, &flags, &appointment);
        if (res != BLE_OK) {
            fprintf(stderr, "host-sim: failed to decode appointment body: %s\n",
                    ble_result_name(res));
            host_sim_canvas_fill(canvas, 200, 0, 0);
            host_sim_canvas_apply_round_mask(canvas);
            return 3;
    ...
    case BLE_SCREEN_NAVIGATION: {
        ble_nav_data_t nav;
        uint8_t flags = 0;
        const ble_result_t res = ble_decode_nav(payload, length, &flags, &nav);
        if (res != BLE_OK) {
            fprintf(stderr, "host-sim: failed to decode nav body: %s\n", ble_result_name(res));
            host_sim_canvas_fill(canvas, 200, 0, 0);
            host_sim_canvas_apply_round_mask(canvas);
            return 3;
        }
    ...
    case BLE_SCREEN_FUEL_ESTIMATE: {
        ...
        uint8_t flags = 0;
        const ble_result_t res = ble_decode_fuel(payload, length, &flags, &fuel);
        if (res != BLE_OK) {
            fprintf(stderr, "host-sim: failed to decode fuel body: %s\n", ble_result_name(res));
            host_sim_canvas_fill(canvas, 200, 0, 0);
            host_sim_canvas_apply_round_mask(canvas);
            return 3;
        }
    ...
    case BLE_SCREEN_ALTITUDE: {
        ...
        uint8_t flags = 0;
        const ble_result_t res = ble_decode_altitude(payload, length, &flags, &alt);
        if (res != BLE_OK) {
            fprintf(stderr, "host-sim: failed to decode altitude body: %s\n", ble_result_name(res));
            host_sim_canvas_fill(canvas, 200, 0, 0);
            host_sim_canvas_apply_round_mask(canvas);
            return 3;
        }
    ...
    case BLE_SCREEN_BLITZER: {
        ...
        uint8_t flags = 0;
        const ble_result_t res = ble_decode_blitzer(payload, length, &flags, &blitzer);
        if (res != BLE_OK) {
            fprintf(stderr, "host-sim: failed to decode blitzer body: %s\n", ble_result_name(res));
            host_sim_canvas_fill(canvas, 200, 0, 0);
            host_sim_canvas_apply_round_mask(canvas);
            return 3;
        }
        host_sim_render_blitzer(canvas, &blitzer, flags);
        return 0;
    }
    case BLE_SCREEN_INCOMING_CALL: {
        ble_incoming_call_data_t call;
        uint8_t flags = 0;
        const ble_result_t res = ble_decode_incoming_call(payload, length, &flags, &call);
        if (res != BLE_OK) {
            fprintf(stderr, "host-sim: failed to decode incoming call body: %s\n",
                    ble_result_name(res));
            host_sim_canvas_fill(canvas, 200, 0, 0);
            host_sim_canvas_apply_round_mask(canvas);
            return 3;

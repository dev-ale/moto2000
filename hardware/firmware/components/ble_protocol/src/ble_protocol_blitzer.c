...
ble_result_t ble_encode_blitzer(const ble_blitzer_data_t *blitzer, uint8_t flags, uint8_t *out_buf,
                                size_t out_cap, size_t *out_written)
{
    ...
    ble_write_header(out_buf, BLE_SCREEN_BLITZER, flags, (uint16_t)BLE_PROTOCOL_BLITZER_BODY_SIZE);

...
static void put_pixel_safe(host_sim_canvas_t *canvas, int x, int y, uint8_t r, uint8_t g, uint8_t b)
{
...
static void draw_fat_line(host_sim_canvas_t *canvas, int x0, int y0, int x1, int y1, int thickness,
                           ...
                           uint8_t flags)
{
    ...
    /* ---------------- hero speed digits ---------------- */
    char speed_buf[4];
    (void)host_sim_format_speed_kmh(data->speed_kmh_x10, speed_buf, sizeof(speed_buf));
    ...
    int speed_x = 0;
    int speed_y = 0;
    host_sim_speed_digit_origin(canvas->width, canvas->height, (int)strlen(speed_visible),
                                speed_scale, &speed_x, &speed_y);
    /* Push down a bit so the heading block at the top has breathing room. */
    speed_y += 30;
    host_sim_draw_text(canvas, speed_visible, speed_x, speed_y, speed_scale, fg_r, fg_g, fg_b);
    ...
    host_sim_draw_text(canvas, unit, unit_x, unit_y, unit_scale, muted_r, muted_g, muted_b);

    /* ---------------- heading block (upper-right) ---------------- */
    char head_buf[8];
    (void)host_sim_format_heading_label(data->heading_deg_x10, head_buf, sizeof(head_buf));
    ...
    host_sim_draw_text(canvas, head_buf, head_x, head_y, head_scale, accent_r, accent_g, accent_b);
    ...
    int arrow_tip_x = 0;
    int arrow_tip_y = 0;
    host_sim_heading_arrow_endpoint(data->heading_deg_x10, arrow_cx, arrow_cy, arrow_len,
                                    &arrow_tip_x, &arrow_tip_y);
    draw_fat_line(canvas, arrow_cx, arrow_cy, arrow_tip_x, arrow_tip_y, 4, accent_r, accent_g,
                  accent_b);

    /* ---------------- altitude (bottom-left) ---------------- */
    char alt_buf[16];
    (void)host_sim_format_altitude_label(data->altitude_m, alt_buf, sizeof(alt_buf));
    const int alt_scale = 3;
    const int alt_x = 60;
    const int alt_y = canvas->height - 8 * alt_scale - 80;
    host_sim_draw_text(canvas, alt_buf, alt_x, alt_y, alt_scale, muted_r, muted_g, muted_b);

    /* ---------------- temperature (bottom-right) ---------------- */
    char temp_buf[16];
    (void)host_sim_format_temperature_label(data->temperature_celsius_x10, temp_buf,
                                            sizeof(temp_buf));
    ...
    host_sim_draw_text(canvas, temp_buf, temp_x, temp_y, temp_scale, muted_r, muted_g, muted_b);
